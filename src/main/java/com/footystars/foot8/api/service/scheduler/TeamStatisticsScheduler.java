...
public class TeamStatisticsScheduler {
        ...
        private final TeamStatsFetcher teamFetcher;
        private static final Logger log = LoggerFactory.getLogger(TeamStatisticsScheduler.class);

        private static final String FETCHED = "Fetched team statistics";
        private static final String FETCHED_ERROR = "Fetched {} failed";
//    @Scheduled(cron = "0 39 20 * * *")
        public void getPremierLeagueTeamsInfo() {
            try {
                teamFetcher.fetchCurrentSeasonsTeam();
                log.info(FETCHED);
            } catch (Exception e) {
